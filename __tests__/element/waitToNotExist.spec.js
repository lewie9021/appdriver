...
it("throws an ElementWaitError if the polling times out", async () => {
  ...
  try {
    ...
    expect(err).toHaveProperty("message", `Element still found after 5000ms timeout (interval: 200ms).`);
  }
...
it("supports passing a 'maxDuration' parameter", async () => {
  ...
  try {
    ...
    expect(err).toBeInstanceOf(ElementWaitError);
    expect(err).toHaveProperty("message", `Element still found after ${maxDuration}ms timeout (interval: 200ms).`);
  }
...
it("supports passing a 'interval' parameter", async () => {
  ...
  try {
    ...
    expect(err).toBeInstanceOf(ElementWaitError);
    expect(err).toHaveProperty("message", `Element still found after 5000ms timeout (interval: ${interval}ms).`);
  }
