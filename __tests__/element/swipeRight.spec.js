...
it("executes the 'swipeElement' method on the Appium Service", async () => {
  ...
  await element(by.label("list-item")).swipeRight({ distance });

  expect(appiumService.findElement).toHaveBeenCalledTimes(1);
  expect(appiumService.swipeElement).toHaveBeenCalledTimes(1);
  expect(appiumService.swipeElement).toHaveBeenCalledWith(expect.objectContaining({ element: ref, distance, direction: 90 }));
});
...
it("supports passing 'x' and 'y' parameters to offset from the top left of the element", async () => {
  ...
  await element(by.label("list-item")).swipeRight({ x, y, distance: 100 });
...
it("support passing a 'percentage' parameter to calculate the swipe distance", async () => {
  ...
  await element(by.label("list-item")).swipeRight({ percentage });
...
it("supports passing a 'duration' parameter to redefine the swipe duration", async () => {
  ...
  await element(by.label("list-item")).swipeRight({ duration, distance: 100 });
...
it("throws an ElementActionError for Appium request errors", async () => {
  ...
  try {
    ...
    expect(error).toHaveProperty("message", "Failed to swipe right on element.");
  }
...
it("propagates other types of errors", async () => {
  ...
  await expect(element(by.label("list-item")).swipeRight({ distance: 100 }))
