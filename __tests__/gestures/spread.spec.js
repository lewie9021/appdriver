...
it("returns a Gesture instance", () => {
  const gesture = gestures.spread({ x: 100, y: 100, distance: 25 });

  expect(gesture).toBeInstanceOf(Gesture);
});

it("correctly resolves to a spread gesture", () => {
  const x = 200;
  const y = 300;
  const distance = 25;
  const gesture = gestures.spread({ x, y, distance });
...
it("supports passing a duration parameter", () => {
  const duration = 2000;
  const gesture = gestures.spread({ x: 100, y: 100, distance: 200, duration });
...
it("supports passing a direction parameter", () => {
  ...
  const y = 100;
  const distance = 25;
  const gesture = gestures.spread({ x, y, distance, direction: 180 });
