...
it("returns a Gesture instance", () => {
  const gesture = gestures.swipeUp({ x: 100, y: 100, distance: 200 });

  expect(gesture).toBeInstanceOf(Gesture);
});

it("correctly resolves to a swipe up gesture", () => {
  const x = 200;
  const y = 300;
  const distance = 100;
  const gesture = gestures.swipeUp({ x, y, distance });
...
it("supports passing a duration parameter", () => {
  const duration = 2000;
  const gesture = gestures.swipeUp({ x: 100, y: 100, distance: 200, duration });
