...
it("executes the 'dismissAlert' method on the Appium Service", async () => {
  const text = [ "Alert", "Hello World!" ].join("\n");

  jest.spyOn(appiumService, "getAlertText").mockResolvedValue(text);
  jest.spyOn(appiumService, "dismissAlert").mockResolvedValue(null);
  ...
  expect(appiumService.dismissAlert).toHaveBeenCalledTimes(1);
...
it("throws an ActionError if no alert is present", async () => {
  ...
  try {
    ...
    expect(err).toHaveProperty("message", "Failed to dismiss alert. No alert present.");
  }

  expect(appiumService.getAlertText).toHaveBeenCalledTimes(1);
  expect(appiumService.dismissAlert).toHaveBeenCalledTimes(0);
  expect(appiumService.getAlertVisible).toHaveBeenCalledTimes(0);
});

it("throws an ActionError if dismiss the alert fails", async () => {
  const error = new AppiumError("Request error.", 3);
  const text = [ "Alert", "Hello World!" ].join("\n");

  jest.spyOn(appiumService, "getAlertText").mockResolvedValue(text);
  jest.spyOn(appiumService, "dismissAlert").mockRejectedValue(error);
  expect.assertions(5);
  ...
  try {
    ...
    expect(err).toBeInstanceOf(ActionError);
    expect(err).toHaveProperty("message", "Failed to dismiss alert. Alert still present.");
  }
...
it("throws an ActionError if alert is still visible after polling", async () => {
  ...
  try {
    ...
    expect(err).toBeInstanceOf(ActionError);
    expect(err).toHaveProperty("message", "Failed to dismiss alert. Alert still visible after 5000ms.");
  }
